"""FEU Alabang - Department of Computer Engineering Final Laboratory Examination: Multi-Sensor
Avionics Bus Simulator Target Platform: GitHub Codespaces (Linux / Python 3.11+)

DESCRIPTION: This module simulates a high-frequency avionics sensor bus (MIL-STD-1553/ARINC
429). It polls hardware peripherals concurrently, formats telemetry buffers, and executes downstream
signal normalization for flight control algorithms.
"""
...
class AvionicsBusMaster:
    ...
    def _dump_bus_diagnostics(self):
        ...
        for channel, data in self.active_bus_register.items():
            print(
                f"Channel [{channel:12}] -> Packets: {data['packets_received']:2} | "
                f"Latest: {data['latest_voltage']}V | Status: {data['status']}"
            )
